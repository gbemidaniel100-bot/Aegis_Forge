...
def _score_case(case: dict[str, str]) -> tuple[float, str]:
    ...
    keywords = [token.lower() for token in case["expected"].split() if len(token) > 3]
    ...
    return score, f"retrieval matched {found}/{len(keywords)} expected signals"


def evaluate_case(case: dict[str, str], store: Store | None = None) -> dict[str, Any]:
    score, detail = _score_case(case)
    record = {"name": case["name"], "incident": case["incident"], "score": round(score, 4), "detail": detail}
...
def evaluate_suite(store: Store | None = None) -> dict[str, Any]:
    ...
    summary = {"overall": round(overall, 4), "cases": cases}
    if store is not None:
...
def _evaluate_case(case: dict[str, str], store: Store | None = None) -> dict[str, Any]:
    ...
    return {"name": record["name"], "score": record["score"], "detail": record["detail"]}
